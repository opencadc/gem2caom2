...
def _build_chunk_energy(chunk, filter_name, fm):
    ...
    # both sides of the ‘+’.
    bandpass_name = (
        None if len(filter_name) == 0 else filter_name.replace('+', ' + ')
    )
    ...
    if data_product_type == DataProductType.IMAGE:
        ...
        if disperser in [
            ...
            if (
                ...
                fm.resolving_power = NIRI_RESOLVING_POWER[bandpass_name][
                    f_ratio
                ]
...
def _update_chunk_energy_f2(
    chunk, header, data_product_type, obs_id, filter_name
):
...
def _update_chunk_energy_general(
...
def _update_chunk_energy_michelle(
    chunk, data_product_type, obs_id, filter_name
):
    """General chunk-level Energy WCS construction."""
    ...
    if filter_md is None:  # means filter_name not found
        ...
        w_max, w_min = _multiple_filter_lookup(
            filter_name, michelle, obs_id, em.Inst.MICHELLE, wl_max=100
        )
        filter_md = FilterMetadata()
        filter_md.set_bandpass(w_max, w_min)
        filter_md.set_central_wl(w_max, w_min)
...
def _update_chunk_energy_gnirs(
    chunk, data_product_type, obs_id, filter_name, header
):
    ...
    if 'Dark' in filter_name or 'DARK' in filter_name:
        ...
        if data_product_type == DataProductType.SPECTRUM:
            logging.info(f'gnirs: SpectralWCS Spectroscopy mode for {obs_id}.')
            disperser = em.om.get('disperser')
...
def _update_chunk_energy_phoenix(
    chunk, data_product_type, obs_id, filter_name
):
    ...
    if data_product_type in [DataProductType.SPECTRUM, DataProductType.IMAGE]:
        logging.debug(
...
def _update_chunk_energy_flamingos(
    chunk, data_product_type, obs_id, filter_name
):
    ...
    if data_product_type == DataProductType.SPECTRUM:
        ...
        logging.debug(f'Flamingos: SpectralWCS imaging mode for {obs_id}.')
    else:
        raise mc.CadcException(
...
def _update_chunk_energy_hokupaa(
    chunk, data_product_type, obs_id, filter_name
):
    ...
    if 'LowFlx' in filter_name or 'home' in filter_name:
        ...
        if data_product_type == DataProductType.IMAGE:
            logging.debug(f'hokupaa: SpectralWCS imaging mode for {obs_id}.')
            fm = FilterMetadata()
...
def _update_chunk_energy_gmos(
    chunk, data_product_type, obs_id, filter_name, instrument
):
    ...
    if data_product_type == DataProductType.SPECTRUM:
        logging.debug(f'{instrument}: SpectralWCS spectroscopy for {obs_id}.')
        ...
        logging.debug(f'{instrument}: SpectralWCS imaging for {obs_id}.')
        fm = filter_md
    else:
        raise mc.CadcException(
...
def _update_chunk_energy_cirpass(chunk, data_product_type, obs_id):
    ...
    if data_product_type == DataProductType.SPECTRUM:
        logging.debug(f'cirpass: SpectralWCS spectral mode for {obs_id}.')
        fm = FilterMetadata()
...
def _reset_energy(observation_type, data_label, instrument, filter_name):
    ...
    if (
        ...
        or (
            instrument in [em.Inst.GMOS, em.Inst.GMOSN, em.Inst.GMOSS]
...
def _multiple_filter_lookup(
    filter_name, lookup, obs_id, instrument, wl_max=None
):
