...
def test_subset_all():
    ...
    assert temp[0].startswith(
        ...
        'wrong content'
    assert len(list(temp)) == 495, 'wrong count'
...
def test_subset_only_end():
    ...
    assert temp[0].startswith(
        ...
        'wrong content'
    assert len(list(temp)) == 397, 'wrong count'
...
def test_is_processed():
    ...
    for ii in tests:
        if ii == 'rS20121030S0136':
            assert GemObsFileRelationship.is_processed(
                ii, em.Inst.OSCIR) == tests[ii], 'failed {}'.format(ii)
        else:
            assert GemObsFileRelationship.is_processed(
                ii, em.Inst.GMOS) == tests[ii], 'failed {}'.format(ii)
...
def test_repair_data_label():
    ...
    for ii in test_main_app.LOOKUP:
        if ii == 'rgS20100316S0366':
            test_result = em.gofr.repair_data_label(
                ii, em.Inst(test_main_app.LOOKUP[ii][1]),
                test_main_app.LOOKUP[ii][0], 1)
        else:
            test_result = em.gofr.repair_data_label(
                ii, em.Inst(test_main_app.LOOKUP[ii][1]),
                test_main_app.LOOKUP[ii][0])
